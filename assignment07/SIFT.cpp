...
bool ratio_threshold = false;
	...
	imwrite("result.jpg", matchingImage);
...
int nearestNeighbor(const Mat& vec, const vector<KeyPoint>& keypoints, const Mat& descriptors) {
	...
	for (int i = 0; i < descriptors.rows; i++) {
		...
		//	Fill the code
		//
		double dist = euclidDistance(vec, v);	// distance
		if (dist < minDist) {	
			minDist = dist;		// minDist replace
			neighbor = i;		// replace neighbor index
		}
...
void findPairs(vector<KeyPoint>& keypoints1, Mat& descriptors1,
	...
	for (int i = 0; i < descriptors1.rows; i++) {
		...
		if (ratio_threshold) {
			// Define correct match based on 1st closest neighbor~2nd closest neighbor 

			double closest = euclidDistance(desc1, descriptors2.row(nn));
			double secondClosest = 1e6;
			for (int j = 0; j < descriptors2.rows; j++) {
				if (j == nn) {	// skip 1st closest neighbor
					continue;
				}
				double temp = euclidDistance(desc1, descriptors2.row(j));	// find 2nd closest
				if (temp < secondClosest) secondClosest = temp;
			}
			if (closest > RATIO_THR * secondClosest) {
				continue;	// skip if 1st, 2nd closest neighbor ratio is larger than threshold
			}
		}
		...
		if (crossCheck) {
			...
			if (rev != i) {
				continue;	// skip if the points are not mutual nearest neighbors
			}
		}
