		...
		for (int j = 0; j < sq_col; j++) {
			...
			if ((y >= 0) && (y <= (row - 1)) && (x >= 0) && (x <= (col - 1))) {
				if (!strcmp(opt, "nearest")) {
					...
					int x_nearest = round(x);
					int y_nearest = round(y);

					output.at<T>(i, j) = input.at<T>(y_nearest, x_nearest);
				}
					...
					output.at<T>(i, j) = 
    (1 - lambda) * (1 - mu) * input.at<T>(y1, x1) + 
    lambda       * (1 - mu) * input.at<T>(y1, x2) + 
    (1 - lambda) * mu       * input.at<T>(y2, x1) + 
    lambda       * mu       * input.at<T>(y2, x2);
